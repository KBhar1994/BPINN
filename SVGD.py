...
class SVGD(object):
    ...
    def __init__(self, bayes_nn,train_loader):
        ...
        self.optimizers, self.schedulers = self._optimizers_schedulers(lr, lr_noise)
    ...
    def train(self, epoch,data_likelihod, eq_likelihood, rec_lamda, rec_beta_eq, rec_log_beta, rec_log_alpha,LOSS,LOSSB,LOSS1,LOSS2,LOSS3,LOSSD):
        ...
        for batch_idx, (x, y) in enumerate(self.train_loader): # loop over the data batch
            ...
            self.bayes_nn.zero_grad() # set the gradient to zero
            ...
            for i in range(self.n_samples): # loop over all ensembles
                #####################
                ###modified for sparse data stenosis
                ## forward for training data
                
                # All the .forward() statements below takes in a (nx1) tensor and returns a (nx3) tensor due to the structure of the NNs
                ...
                log_eq_i,loss_1i,loss_2i,loss_3i =  self.bayes_nn.criterion(i,x,y,ntrain) # 'ntrain' is the size of total training dataset
                ...
                log_joint_i.backward() # computing gradients via back propagation
                if (i==0) :
